...
export default function PropertyCardItem({
  ...
  const imageValue = property.image ?? property.images;
  const images = (Array.isArray(imageValue) ? imageValue : imageValue ? [imageValue] : []).filter(Boolean);
      ...
      <Box sx={{ display: "flex", gap: 1, overflowX: "auto", scrollSnapType: "x mandatory" }}>
        {images.length > 0 ? images.map((image, index) => (
          ...
          >
            ...
            <Box
              hidden
              sx={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 1, color: "text.secondary" }}
            >
              <BrokenImageOutlinedIcon fontSize="large" />
              <Typography>Image could not be loaded</Typography>
            </Box>
          </Box>
        )) : (
